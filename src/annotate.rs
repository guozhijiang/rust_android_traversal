...
impl Canvas {
    ...
    fn blend(&mut self, x: i32, y: i32, c: (u8, u8, u8), a: f32) {
        ...
        let eff = a * alpha;
        for (i, cv) in [c.0, c.1, c.2].into_iter().enumerate() {
            let cur = px.0[i] as f32;
            px.0[i] = (cur * (1.0 - eff) + cv as f32 * eff) as u8;
        }
        if px.0[3] < 255 {
            px.0[3] = 255;
        }
        ...
        match m.kind {
            ...
            MarkerKind::Swipe => {
                ...
                if m.points.len() >= 2 {
                    ...
                    self.badge(
                        from.0 as f32 + 24.0 * s,
                        from.1 as f32 - 24.0 * s,
                        m.number,
                        c,
                    );
                }
...
fn glyph(ch: char) -> Option<[u8; 7]> {
    let rows: [&str; 7] = match ch {
        ...
        'Q' => [
            "01110", "10001", "10001", "10001", "10101", "10011", "01101",
        ],
        'R' => [
